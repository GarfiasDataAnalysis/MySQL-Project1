...
-- display name of actor of whom you only know the nae Joe
SELECT 
...
FROM
    actor;
ALTER TABLE actor
ADD COLUMN description blob(200) AFTER first_name;
...
SELECT 
    first_name, last_name, address, city.city, country.country
FROM
    staff
        INNER JOIN
...
SELECT 
    title, COUNT(actor_id) AS numbeofActors
FROM
    film
        INNER JOIN
...
SELECT 
    title, COUNT(inventory_id) AS numberofCopies
FROM
    film
        INNER JOIN
...
SELECT 
    film.title, COUNT(*) AS 'rent count'
FROM
    film,
    inventory,
    rental
